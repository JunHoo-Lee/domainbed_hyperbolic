...
class hypermean(nn.Module):
    def __init__(self, c=0.1,clip_r=1):
        super(hypermean, self).__init__()
        self.c = c
        #self.topoincare = ToPoincare(c = self.c, clip_r=2.3)
        self.topoincare = ToPoincare(c = self.c)

    def forward(self, x1, x2,lam=1, topoin= False, c=None):
        llam = 1 - lam
        if c is None:
            c = self.c
        #if topoin:
        #    x1 = self.topoincare(x1)
        #    x2 = self.topoincare(x2)

        return pmath.mobius_add(pmath.mobius_scalar(x1,lam,c), pmath.mobius_scalar(x2 ,llam,c))




class HyperbolicDistanceLayer(nn.Module):
    def __init__(self, c, clip_r=1):
        super(HyperbolicDistanceLayer, self).__init__()
        self.c = c
        self.clip_r = clip_r
        self.topoincare = ToPoincare(c = self.c, clip_r= self.clip_r)

    def forward(self, x1, x2, topoin= False,lam=None, c=None):
        if c is None:
            c = self.c
        #if topoin:
        #    x1 = self.topoincare(x1)
...
class ToPoincare(nn.Module):
    ...
    def forward(self, x):
        if self.clip_r is not None:
            ...
            fac1= torch.maximum(
                torch.ones_like(x_norm), 
                self.clip_r*0.01 / x_norm
            )
            #print("x_norm", x_norm)
            #x = x * torch.maximum(fac,fac1)
            x = x * fac
        # **2 고칠것 
#        if self.train_x:
#            xp = pmath.project(pmath.expmap0(self.xp, c=torch.abs(self.c), c=self.c**2)
#            return self.grad_fix(pmath.project(pmath.expmap(xp, x, c=self.c**2), c=self.c**2))
        #return self.grad_fix(pmath.project(pmath.expmap0(x, c=abs(self.c)+1e-4), c=abs(self.c)+1e-4)), x_norm
        return self.grad_fix(pmath.project(pmath.expmap0(x, c=abs(self.c)+1e-4), c=abs(self.c)+1e-4))
