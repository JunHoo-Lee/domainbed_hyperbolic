...
def _hparams(algorithm, dataset, random_seed):
    ...
    _hparam("c", 0.1, lambda r: r.choice([0.1, 0.01, 0.05]))
    _hparam("clip_r", 1, lambda r: r.choice([1.0, 2, 0.5]))
    _hparam("add_c", 1, lambda r: r.choice([1, 0.1, 0.1]))
    _hparam("hyp_method", 1, lambda r: r.choice([0, 0.1, 0.5]))
    _hparam("class_balanced", False, lambda r: False)
    _hparam("hyperbolic_classifier", True, lambda r: True)
    _hparam("radius_reg", 0.001, lambda r: r.choice([0.0, 0.1, 0.01, 0.001]))
    # TODO: nonlinear classifiers disabled
    _hparam("nonlinear_classifier", False, lambda r: bool(r.choice([False, False])))
    _hparam("method", "exp", lambda r: r.choice(["exp", "exp_tan", "tan_tan"]))
